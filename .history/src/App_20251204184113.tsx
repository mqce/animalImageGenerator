...
const COMMON_ELEMENTS = {
  ...
  shooting: [
    ...
    */
    ...
    "Vintage photograph from the 90s",
  ...
  negative: [
    ...
    "細かく描写された手足の指",
    "撮影者の指などの余計な写りこみ",
    "mirror image", "perfect anatomy", "3d render",
    "professional photography", "bokeh", "depth of field", "studio lighting", "hd", "4k", "sharp focus", "perfect composition", "luxurious texture",
    "date stamp", "timestamp", "watermark", "text", "numbers", "digital display",
  ]
};
...
const VARIABLE_ELEMENTS = {
  ...
  pattern: [
    ...
    "狐のような色",
    "猫のような模様",
    "Asymmetrical markings（左右非対称の模様）",
  ],
  action: [
      ...
      const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${apiKey}`, {
        ...
        body: JSON.stringify({
          contents: [{
            parts: [{
              text: `
あなたは偶然そこに居合わせた動物を見つけて、安物のカメラで撮影します。
...
## 制約事項
1. **実在感の追求**: ファンタジーやアニメ調ではなく、偶然撮影したようなリアルな写真に見えるように描写してください。
2. **背景と環境**: 動物の「現在の状態（動作）」に合わせて、最適な背景、地面、周囲の環境を詳細に描写してください。
3. **撮影条件**: 
   ${COMMON_ELEMENTS.shooting.join('\n   ')}
4. **構図**: 写真の中に動物をどの位置に、どれくらいの大きさで、どの角度で配置すると良いかを考慮してください。
5. **禁止事項**: ${COMMON_ELEMENTS.negative.join(', ')} は含めないでください。
